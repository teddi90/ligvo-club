...
const sliderGameCards = computed(() => {
    return store.getMtgGames.filter(
...
const paginate = computed(() => {
    if (getAmountOfUsedFilterOptions.value) {
        ...
        return store.getMtgGames.slice(
            (currentPage.value - 1) * pageSize.value,
            currentPage.value * pageSize.value
        );
});
const getAmountOfPages = computed(() => {
    ...
    } else return Math.ceil(store.getMtgGames.length / pageSize.value);
});
